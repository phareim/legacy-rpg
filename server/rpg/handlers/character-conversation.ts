...
export async function handleCharacterConversation(
    ...
    if (!character) {
        ...
        character = await getCharacter(characterName, db, bucket)
    }
...
- Referencing past: "Last time we spoke, you mentioned the ancient ruins. Did you ever find them?"`
    ...
    try {
        ...
        await addToCharacterConversation(characterName, userMessage, npcResponse, db, bucket)
...
export async function handleCharacterQuestion(
    ...
    return handleCharacterConversation(characterName, userMessage, gameState, openai, db, bucket)
}
