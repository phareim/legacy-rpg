...
import { putR2Json } from '../utils/r2'
...
export default defineEventHandler(async (event: any) => {
    if (event.method === 'DELETE') {
        try {
            ...
            // Clean up R2 blobs
            await Promise.all([
                bucket.delete(`game-states/${userId}/messages.json`),
                bucket.delete(`game-states/${userId}/inventory.json`),
                bucket.delete(`game-states/${userId}/visited.json`)
            ])
            return { success: true }
        } catch (error: any) {
    ...
    try {
        ...
        let gameState = await loadGameState(userId, db, bucket)
        if (!gameState) {
            gameState = { ...DEFAULT_GAME_STATE, visited: ['0,0'] }
            await saveGameState(userId, gameState, db, bucket)
        }
                    ...
                    if (normalizedInventory.includes(normalizedItemName)) {
                        ...
                        try {
                            ...
                            if (!itemRow) {
                                ...
                                // Mark item as picked up in D1
                                await db.prepare(
                                    'UPDATE items SET is_picked_up = 1, updated_at = CURRENT_TIMESTAMP WHERE name = ? AND is_picked_up = 0'
                                ).bind(itemName).run()

                                // Update inventory in R2
                                await putR2Json(bucket, `game-states/${userId}/inventory.json`, updatedInventory)
